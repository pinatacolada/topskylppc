...
\begin{longtable}{|p{3cm}|p{4cm}|p{8cm}|p{6cm}|}
    ...
    * &
    ...
    ASP &
      ...
      If a speed clearance or request is in  progress via CPDLC, displays  “{[}speed{]}”, followed by “+” if the  request or answer contains a reason  (i.e. DUE TO something). &
      If a value is assigned and  HOP: Proposition 
      \smallskip
    ...
    ASSR &
      ...
      If a SQUAWK SSR message is in  progress via CPDLC, displays the  code in brackets, followed by “+” if  the answer contains a reason (i.e.  DUE TO something). 
      \bigskip
    ...
    EST/DEP/ABT &
      ...
      “DEP” when clearance flag set or ground state “DEPA” but not yet departed
    ...
    NPT &
      Next route point &
      When a DCT request or clearance  via CPDLC is in progress: Displays the point name followed by  “+” if the request contains a reason  (i.e. DUE TO something) B : Otherwise: Next point on the route &
      If a CPDLC uplink message  has been sent:  CPDLC UM Clearance If a CPDLC downlink  message has been  received:  CPDLC DM Request If controller timeout has  expired following a CPDLC  downlink request:  CPDLC Controller Late If a CPDLC warning has  been raised: color of the  warning \\ \hline
    NRAC &
      Number of aircraft &
      Number from 2 to 99 Extended label: “n” if 1 Other label or (List): blank if 1 Number can be set by prefixing the  aircraft type in the FPL by “X/”  where X is the number (max 99). &
       \\ \hline
    NSSR &
      SSR warning &
      Displays ASSR if different from TSSR &
      A : Warning If a CPDLC uplink message  has been sent:  CPDLC UM Clearance If a CPDLC warning has  been raised: color of the  warning \\ \hline
    OP\_TEXT &
    ...
    P &
      ...
      “P” if aircraft equipment is not  indicating P-RNAV/RNAV1 capability B : …and destination has arrival  procedures requiring it &
      A : Not equipped: Urgency Unknown: Unknown B : Information \\ \hline
    PEL &
      Planned Entry Level &
      “CA” if Clear for App set, “VA” if Visual App flag set, See AFL field for number format. &
      Ongoing coordination:  Proposition Change just before ETN: Info Coord Coordination refused: Warning \\ \hline
    PFREQ &
    ...
    PRFL &
      ...
      If a CPDLC downlink  message has been  received:  CPDLC DM Request If controller timeout has  expired following a CPDLC  downlink request:  CPDLC Controller Late If a CPDLC warning has  been raised: color of the  warning \\ \hline
    PSSR &
    ...
    R &
      ...
      Not equipped: Urgency Unknown: Unknown \\ \hline
    RFL &
      Requested Flight Level &
      See AFL field for format (List): If an RFL request is in progress via  CPDLC, displays “{[}requested level{]}”,  followed by “+” if the request  contains a reason (i.e. DUE TO  something). (DEP list) does not display the  brackets for CPDLC status. &
      In flight lists: If a CPDLC downlink  message has been  received:  CPDLC DM Request If controller timeout has  expired following a CPDLC  downlink request:  CPDLC Controller Late If a CPDLC warning has  been raised: color of the  warning \\ \hline
    RI &
      Release Indicator &
      “F” fully released “C” released for climb “D” released for descent “T” released for turns Incoming release disappears 3 min  after track is assumed, outgoing  when track is no longer redundant. &
      During transfer:  Proposition \\ \hline
    ROUTE &
      Flight plan route &
      (DEP list) - part 1 shows first 15 characters,   part 2 characters 16-30 If departing from defined airports: - first 30/45 characters (2 or 3 parts) If arriving at a clearance flag airport: - last 30/45 characters (2 or 3 parts) Else: - first 30/45 characters (2 or 3 parts) &
       \\ \hline
    S &
    ...
    SI &
      Sector Indicator &
      Assumed track: Next sector identifier or frequency.  Displayed in brackets if a  communications transfer is in  progress via CPDLC, followed by “+”  if the answer contains a reason (i.e.  DUE TO something). The frequency  display is forced on during a  transfer via CPDLC. Other tracks: Tracking controller identifier or  frequency. (List): frequency display is not possible (DEP list) does not display the  brackets for CPDLC status. (Current) displays always the  current tracking controller id (Previous) displays the current  tracking controller id unless it is you  or the track is in redundant state (Next) displays the next controller id  when assumed, or current when in  redundant state &
      Priority order: Outgoing HOP: Proposition Manually changed next  sector: Warning Normal next sector:  Coordination If a CPDLC communications  transfer uplink message  has been sent:  CPDLC UM Clearance If a CPDLC warning has  been raised: color of the  warning \\ \hline
    SID &
      Assigned/planned SID &
      SID identifier &
      If automatically assigned:  Sid Star No Allocation If manually assigned: Sid Star Allocation (optional feature) \\ \hline
    SQ &
    ...
    TEXT2 &
      OP-TEXT2 message &
      If more than 10 characters, then   shows first 9 + “\textgreater{}” &
       \\ \hline
    TRACK &
      Ground track &
      “T” + the aircraft’s current ground  track in degrees (true): true (magnetic): magnetic &
       \\ \hline
    TSSR &
    ...
    V &
      Flight rules &
      “V” if VFR, “Y” if first IFR and later VFR, “Z” if first VFR and later IFR &
       \\ \hline
    W &
      No RVSM capability &
    ...
    WTC &
      ...
      “J” for Super, “H” for Heavy, “M” for Medium, “L” for Light “?” for unknown B : Unselected label: Only displayed if not medium unless  the field is highlighted &
      If highlighted: Warning \\ \hline
    /WTC &
      Wake turbulence  category &
      “/J” for Super, “/H” for Heavy, “/M” for Medium, “/L” for Light “?” for unknown B : Unselected label: Only displayed if not medium unless  the field is highlighted &
