...
There are 8 different Sector States. Track presentation coloring depends on the flight sector state:

\begin{tabular}{l l l}
  \textbf{State}                                                  & \textbf{Color}               & \textbf{Condition}\\
  Unconcerned \nextrow \label{state:Unconcerned}                  & \colortabex{Unconcerned}     & Track will not enter the active sector\\
  Notified \nextrow \label{state:Notified}                        & \colortabex{Concerned}       & Track will enter the active sector (> 15 min)\\
  Coordinated \nextrow \label{state:Coordinated}                  & \colortabex{Coordination}    & Track will enter the active sector (< 15 min)\\
  Assumed \nextrow \label{state:Assumed}                          & \colortabex{Assumed}         & Track is assumed\\
  On Contact \nextrow \label{state:On Contact}                    & \colortabex{Assumed}         & Track is on-contact with you\\
  Transfer Initiated \nextrow \label{state:Transfer Initiated}    & \colortabex{Assumed}         & Track is being transferred to the next controller\\
  Redundant \nextrow \label{state:Redundant}                      & \colortabex{Redundant}       & Track has been transferred to the next controller but is still inside the active sector\\
  Free \nextrow \label{state:Free}                                & \colortabex{Unconcerned}     & Track is not assumed or on-contact with anyone\\
\end{tabular}
...
\begin{table}[H]
  ...
  \end{tabular}
  \caption{Flight Plan transferred from NORL to CENL and less than 15 minutes to SULL}
...
\begin{table}[H]
  ...
  Sector                                  & NORL                 & CENL               & SULL                \\ \hline
  Aircraft Position                       &                      & \usym{2708}        &                     \\ \hline
  Sector State                            & \stateref{Unconcerned} & \stateref{Redundant} & \stateref{Assumed} \\ \hline
  \end{tabular}
  \caption{Flight Plan transferred from CENL to SULL}
  \label{tab:state-example-exit}
\end{table}
...
\begin{tabular}{l l l}
\image{img/rps_fpasd.png}     & Flight plan track                   & Position is not based on surveillance data but calculated by EuroScope \\
\image{img/rps_coast.png}     & Coasted track                       & No position updates in over 30 seconds, position no longer reliable \\
\image{img/rps_psr.png}       & Primary radar track                 & \\
\image{img/rps_sec.png}       & Secondary or combined radar track   & Uncontrolled flight\\
\image{img/rps_psr+ssr.png}   & Secondary or combined radar track   & Controlled flight \\
%                              & ADS-B only track                    & \\
\end{tabular}
...
\begin{tabular}{l l}
\image{img/rps_spi.png} & Secondary radar track without DAPs with Special Position Indication
\end{tabular}
...
\begin{tabular}{l l}
\image{img/rps_divalert.png}& Secondary radar track without DAPs with divergence alert
\end{tabular}
...
The length of the prediction line can be changed for all tracks via the \winref{win:tcw}, or for a single track via the \winref{menu:prl}. The example below shows a selected track with 5 history dots and a 3-minute prediction line. Prediction lines are not displayed for flight plan tracks.

\image{img/rps_predline+history_selected.png}
...
The Uncontrolled List is a EuroScope-style list containing all or a subset of Uncontrolled aircraft, depending on whether filters are set up.
...
\begin{longtable}{|p{2.5cm}|p{2.5cm}|p{4.5cm}|p{4.5cm}|}
    ...
    * \nextrow \label{tag:*}&
    ...
    ALRT \nextrow \label{tag:ALRT}&
      ...
      MSAW, APW and MTCD: \colorref{color:Warning} 
