...
\usepackage[hmargin=2cm,top=4cm,headheight=85pt,footskip=55pt]{geometry}
...
\newcommand{\headerformat}{%
  ...
  \begin{tabular}{|P{3cm}|P{10cm}|P{2cm}|} 
    \hline
    \vspace{0.01cm}\includegraphics[width=2.5cm]{img/logo.png} & \vspace{0.01cm}\textbf{\large{\leftmark}} & \vspace{0.01cm}\textbf{\large{\thesection}}  \\
...
\vtop{%
\centering
\includegraphics[width=8cm,height=8cm,keepaspectratio]{#1}%
\captionof{figure}{#2}%
}%
\end{center}
\ignorespaces}
...
\newenvironment{Warn}
  {\begin{shaded}\reversemarginpar\marginnote{\colorbox{red}{\color{white}Warning}}}
  {\end{shaded}}

  \newenvironment{Note}
  {\begin{shaded}\reversemarginpar\marginnote{\colorbox{gray}{\color{white}Note}}}
  {\end{shaded}}
...
% PDF VIEW
 ...
 bindingoffset=0mm, top=40mm,bottom=25mm}
% PRINT
%\geometry{total={210mm,297mm},
...
\begin{Warn}
  IF MORE THAN ONE POSITION ASSIGNS ITSELF AS "CDM MASTER" A-CDM WILL NOT BE OPERATIONAL
\end{Warn}
...
\begin{Warn}
  ROF should not be used to request an early handover of a flight. ROF is a coordination tool that is context dependent, using it outside the prescribed scenarios or ir other unintended ways will cause unnecessary confusion.
\end{Warn}
...
\begin{Note}
  If a ROF is sent to a controller not using the TopSky plugin or the message fails to go through, an error message will be put into the Personal Queue Window. The message counter in the Global Menu will be highlighted in \colorref{Warning} color if the window is not open.
\end{Note}
...
\begin{Warn}
  Attempting to connect with a CPDLC login callsign that is already in use will not fail instantly, and may take up to 10 seconds for the error message to be added to the \winref{win:pqw}
\end{Warn}
...
TopSky may sometimes fail to parse the message contents correctly. The longer the message, the more likely it is that the parsing will go wrong. When in doubt, open the message in the \winref{win:dlcmw}.
...
\begin{Note}
  While it is possible to answer any message in the \winref{win:dlcmw} using the “Manual Reply” option, the track label menus should always be used to answer correctly recognized messages, as using the manual reply method will not update the track label values.
\end{Note}
...
The List CLR tag item will show a “R” text in yellow (red if the request contained remarks text after the ATIS indicator. Check the message in the \winref{win:dlcmw}. The text will start to blink two minutes before a timeout occurs if a clearance hasn’t been sent.
...
\begin{longtable}{|p{5cm}|p{5cm}|p{2.5cm}|}
  \hline
...
MTCD track label field Either “•” in \colorref{Urgency} color or “MTCD” in \colorref{Warning} color
...
SAP conflicts (optionally also risks) are also shown in the \winref{win:sap}.
...
The STCA system is designed to alert the controller of a possible or actual loss of separation between aircraft. The alert is given a specified time (for example 90 seconds) before a loss of separation is predicted to happen, but it is dependent on the relative positions and movement of the aircraft. The alert will not be shown if both aircraft have STCA alerting inhibited (see the \winref{win:snsw}), are inside exclusion areas for parallel approaches, or when either aircraft is inside an active STCA inhibit area.
...
An alert is shown by coloring the CALLSIGN item background (plugin drawn windows) or the text itself (elsewhere where TopSky can’t control the background color) in \colorref{Urgency} color. A one-minute-long prediction line is displayed in \colorref{Urgency} color regardless of the prediction line settings. 
...
\begin{tabular}{| p{2.5cm} p{2.5cm} p{8cm} |}
  ...
  Transfer Initiated                                              & \colortabex{Assumed}         & Track is being transferred to the next controller. \tagref{tag:CALLSIGN} and \tagref{tag:SI} are colored in \colorref{Redundant} color\\ \hline
...
\begin{longtable}{p{5cm} p{7.5cm}}
  ...
  STCA alert              & Selects which aircraft display the STCA alert:\\                          & \hspace{10pt}• All All aircraft\\                          & \hspace{10pt}• Own+Co Only assumed and coordinated aircraft\\                          & \hspace{10pt}• Own Only assumed aircraft\\
  STCA alert sound        & Selects which STCA alerts trigger the alert sound:\\                          & \hspace{10pt}• All All alerts\\                          & \hspace{10pt}• Own+Co Only alerts with assumed and/or coordinated aircraft involved\\                          & \hspace{10pt}• Own Only alerts with assumed aircraft involved\\
  APW alert               & Selects which aircraft display the APW alert:\\                          & \hspace{10pt}• All All aircraft\\                          & \hspace{10pt}• Own+Co Only assumed and coordinated aircraft\\                          & \hspace{10pt}• Own Only assumed aircraft\\
  APW alert sound         & Selects which STCA alerts trigger the alert sound:\\                          & \hspace{10pt}• All All alerts\\                          & \hspace{10pt}• Own+Co Only alerts for assumed or coordinated\\                          & aircraft\\                          & \hspace{10pt}• Own Only alerts for assumed aircraft\\
  METAR source            & Selects the METAR data source for TopSky windows that display METAR data\\
  FPASD filter            & Allows filtering of displayed FPASD tracks based on sector state\\                          & \hspace{10pt}• Coord Display tracks at least in the coordinated state\\                          & \hspace{10pt}• Conc Display tracks at least in the concerned state\\                          & \hspace{10pt}• None Display all tracks\\
\end{longtable}
\medskip
...
\begin{longtable}{p{5cm} p{7.5cm}}
  ...
  Traffic Mgmt. List 1…       & Opens the \textit{\titleref{list:tml}}               \\
  ...
  Traffic Mgmt. List 2…       & Opens the \textit{\titleref{list:tml}}               \\
...
When enabled, the list options bar displays “Info Conc Redu Filter Filter” on the right edge of the Global Menu. The first three toggle the respective settings for the Sector List and are colored with the appropriate color when enabled, and the last two are displayed in \colorref{VFR} color when the corresponding Uncontrolled list is somehow filtered. Clicking on them opens the Flight Plan Lists submenu to change the filtering options.
...
\label{win:hop}
A Handover Proposal can be used to propose non-standard transfer parameters (AHDG/Direct-to and ASP) to the next sector. For the receiving controller a HOP is identified by coloring the callsign data field with \colorref{Proposition In} color in the label. For the sending controller the Callsign field remains “Assumed” color and the Sector Indicator field is shown in \colorref{Proposition In} color. Additionally, if there are proposed parameters they are also colored “Proposition” in both controllers’ labels.
...
To answer the RTI/TIP, left-click on the requested parameter shown above the track label or the corresponding message in the \winref{win:miw}. This will open the \winref{menu:ttm}.
...
For “RTI” and “TIP” see the \winref{menu:ahdg}.
...
The Combined Transfer menu displays the proposed transfer parameters for a HOP. It is opened by clicking on the AHDG, ASP or COPN/COPX items in the track label or flight list, or the list row displaying the HOP message in the \winref{win:miw}.
...
The Tactical Transfer menu is used to accept, reject or apply tactical data (AHDG, ASP and/or ARC). It is opened by left-clicking on a proposed or accepted parameter in the track label. The menu displays all proposed (\colorref{Proposition In} color) and accepted (sector state color) values.
...
The “AD\_Hotspots” map in the “Aerodromes” folder is an automatically created map that contains aerodrome symbols that are used to open the \winref{win:riuw} and \winref{win:ralw}.
...
NAT tracks extracted from downloaded track messages (see \winref{win:nattmw}) can be found in the “NAT” folder. Eastbound tracks are colored East NAT Map, westbound ones West NAT Map.
...
Displays the expanded route of the FPL currently shown in the \winref{win:fpw}. Already passed points are shown with “Overflown” color background. Points still ahead show the estimated time over the point and the calculated flight level (as calculated by EuroScope) below the point name.
...
The PEL and XFL values are displayed in \colorref{Coordination} color, or in “Proposition” if being coordinated. After a refused coordination, the original value is shown in \colorref{Warning} color. The CFL value is displayed as a horizontal line across the screen.
...
\begin{longtable}{p{2.5cm} p{10cm}}
    ...
    CCAMS               & Plugin selected as the code source:\\
                        ...
                        & $\bullet$ \colorref{Warning} color if simulated traffic is not downloaded\\
                        & ESE file or fixed range selected as the code source:\\
                        & $\bullet$ \colorref{Warning} color\\
...
To open the window from the radar screen, right-click on an airport symbol (+) on the radar screen (for the click spots to be active, the “AD\_Hotspots” map in the “Aerodromes” folder of the \winref{win:mapsw} must be active). Only those airports with runways specified have active click spots.
...
The Tactical Info Window displays coordinated tactical data (AHDG, ASP and/or ARC). Proposed data is in “Proposition”, rejected data in “Warning” and accepted data in sector state color. This window only displays the information; to answer or apply the data use the \winref{menu:ttm}.
