...
\begin{longtable}{|p{5cm}|p{8.5cm}|}
  ...
  \texttt{.cdm recover}                                     & In case of of CTD, it tries to recover the latest time status backed up in the server. Example: \texttt{.cdm recover LPPT} \\ \hline
  ...
  \texttt{.cdm atfcm}                                       & Open/close ATFCM window \\ \hline
...
\begin{longtable}{|p{2.5cm}|p{2.5cm}|p{4.5cm}|p{4.5cm}|}
  ...
  FSTS \nextrow \label{tag:FSTS}&  
    Flight Plan Status&
      ...
      FLS-NRA: Flight Suspended due to Not Reported As Airborne.

      \bigskip

      FLS-CDM: Flight Suspended Triggered by CDM.

      \bigskip

      FLS-MR: Flight Suspended due to Mandatory Route.

      \bigskip

      FLS-GS: Flight Suspended due to Ground Stop.

      \bigskip

      DES: Flight is De-Suspended
      ...
      SLC: Slot Cancellation.

      \bigskip

      ATC ACTIV: Flight is already in movement. Automatically set when AOBT is set. &   
        DES, SAM, SRM, SLC and ACT: 
...
\begin{longtable}{|p{4.5cm}|p{4.5cm}|}
  ...
  Squawk code assignment      & Speed request     \\ \hline
  Squawk ident instruction    & MAYDAY/PAN/SQ7500 \\ \hline
  Voice contact instruction   & Position report   \\ \hline
  Free text                   & Free text         \\ \hline
...
The MTCD system is a tool that enables the controller to predict possible future conflicts between aircraft. The look-ahead time (prediction time) is set to 20 minutes and the separation distance that triggers the alert (prediction distance) to 5 NM within the 3 NM separation area, 7 NM within the 5 NM separation area, and 50 NM within the Conventional Separation Area. Additionally, 10 NM is used within LECM, GMMM and GCCC. No MTCD is available within LPPO.
...
\section{Tactical Controller Tool (TCT)}\label{tool:tct}

\subsection{General}

Tactical Controller Tool works between the STCA and MTCD functions, providing alerts based on the aircraft’s tactical trajectory for the next 5 minutes.

\bigskip

In the lateral plane, the aircraft’s predicted trajectory follows either the planned route, an assigned heading clearance, or the current ground track if the aircraft is not found to follow the cleared route/heading.

\bigskip

In the vertical plane, the predictions use the current level for aircraft maintaining their cleared levels, a range of pre-defined vertical speeds for tracks cleared for a vertical maneuver but still in level flight, and an uncertainty buffer around the current vertical speed for aircraft climbing or descending. The predictions assume the aircraft to level off at its cleared level.

\subsection{Alert display}

An alert is shown by displaying the text “TC” (or “TC+” if the aircraft is involved in multiple TCT conflicts) in the TCT label item. The conflict(s) can be visualized by toggling on the TCT legs display by right clicking the "TC" flag.

\capdimage{img/tc.png}{TCT example}

\section{Blind Spot function}\label{tool:bs}

\subsection{General}

The Blind Spot function alerts the controller for level clearances that would create conflicts, by probing specified levels around the aircraft’s current level for conflicts. The function uses by default the TCT parameters, but some of the parameters can be specified to be different for Blind Spot. Conflicts starting within the next three minutes are displayed.

\capdimage{img/bs.png}{BS function example}

\subsection{Alert display}

The Blind Spot function probes the relevant levels when the CFL Menu is opened, and displays the results with colored bars on the right side of the level values. The probe is not refreshed periodically, and as the results are valid for only the probed time, they will be flagged as expired (\colorref{BS Aged} color) after a specified time. For still valid results, the color coding for the probed levels will be:

\bigskip

\begin{tabular}{l l}
  Conflict                                                          & \colorref{BS Warning}       \\
  No conflict, but conflict level(s) between this level and AFL     & \colorref{BS Crossed Level} \\
  No conflict, and no conflict level(s) between this level and AFL  & \colorref{BS No Conflict}   \\
  No calculated data due to probe failure                           & \colorref{BS Unknown}       \\
\end{tabular}
...
\begin{longtable}{|p{2.5cm}|p{2.5cm}|p{4.5cm}|p{4.5cm}|}
  ...
  * \nextrow \label{tag:*}&
  ...
  DBPS \nextrow \label{tag:DBPS}&
    Downlinked barometric pressure setting &
    Barometric pressure setting as downlinked via mode S DAPs. Not available on VATSIM. &
    \\ \hline
  DGS \nextrow \label{tag:DGS}&
    Downlinked ground speed &
    Ground speed as Downlinked via mode S DAPs. Format as in GS field. &
    \\ \hline
  DHDG \nextrow \label{tag:DHDG}&
    Downlinked heading &
    Magnetic heading as Downlinked via mode S DAPs.  
    
    “H” + 3 digits. &
    \\ \hline
  DIAS \nextrow \label{tag:DIAS}&
    Downlinked indicated airspeed &
    Indicated airspeed as Downlinked via mode S DAPs. An estimated value is displayed if upper wind/temp data is available. &
    \\ \hline
  DMACH \nextrow \label{tag:DMACH}&
    Downlinked Mach number &
    Mach number as Downlinked via mode S DAPs.  An estimated value is displayed if upper wind/temp data is available. &
    \\ \hline
  DRC \nextrow \label{tag:DRC}&
    Downlinked rate of  climb/descent &
    Rate of climb/descent as Downlinked via mode S DAPs. 
  ...
  F \nextrow \label{tag:F}&
    ...
    \colorref{Information}\\ \hline
  FASTRWY \nextrow \label{tag:FASTRWY}&
    FAST runway &
    Arrival runway identifier, if not the first runway in a DPA setup &
    \\ \hline
  FASTwarn \nextrow \label{tag:FASTwarn}&
    FAST warning &
    “SQ” for sequence warning &
    \colorref{Warning}\\ \hline
  ...
  FF \nextrow \label{tag:FF}&
    Feeder fix &
    &
    If highlighted: \colorref{AMAN Data Highlight}\\ \hline
  ...
  FMF \nextrow \label{tag:FMF} &
    ...
    \\ \hline
  FSQ \nextrow \label{tag:FSQ} &
    FAST sequence number &
    Sequence number or “MA” for missed approach &
    If sequence warning or missed approach: \colorref{Warning} 
    
    \bigskip

    If arrival runway is not set as active for arrivals: \colorref{FSQ SI}\\ \hline
  ...
  NFREQ \nextrow \label{tag:NFREQ}&
    Next frequency &
    Next frequency for a departing aircraft, to be sent in DCL &
    \\ \hline
  ...
  TAS \nextrow \label{tag:TAS}&
    ...
    \\ \hline
  TCT \nextrow \label{tag:TCT}&
    TCT indicator &
    “TC” if track is involved in one  TCT conflict, “TC+” if in more than one

    \bigskip
    
    Unselected label:
    
    Not displayed if track is subject to an STCA alert &
    \colorref{TCT Warning}\\ \hline
...
% FAST                  & Toggles on/off the FAST function\\
  Blind Spot            & Toggles on/off the Blind Spot function\\
\end{longtable}
...
\begin{longtable}{p{5cm} p{7.5cm}}
  ...
  Sort                        & Selects sorting option                                                          \\
  Display                     & Uncontrolled 1 List state filter                                                \\
  Filter                      & Uncontrolled 1 List units filter                                                \\
  Uncont. List 2…             & Opens the \winref{list:uncon}                                                   \\
  Sort                        & Selects sorting option                                                          \\
  Display                     & Uncontrolled 2 List state filter                                                \\
  Filter                      & Uncontrolled 2 List units filter                                                \\
  Lost List…                  & Opens the \winref{list:lost}                                                    \\ 
  Traffic Mgmt. List 1…       & Opens the \winref{list:tml}                                                     \\
  Sort                        & Selects sorting option                                                          \\
  Display                     & Traffic Management List 1 flight plan state filter                              \\
  ...
  Sort                        & Selects sorting option                                                          \\
  Display                     & Traffic Management List 2 flight plan state filter                              \\
...
Left- or right-clicking the Sort option cycles through the options forward and backward in alphabetical order. This sorting is achieved using a special sorting tag item in the lists that is hidden from view. The sorting only works when the list is sorted according to that item. If the list is sorted according to any other item, this sorting choice will have no effect until the list is again sorted according to the special sorting item (by displaying it in the list, clicking on the column title, and then hiding it again).

\bigskip

Left-clicking the “LFUNC”, “ADEP”, “ADES” and “Via” boxes open text entry boxes to enter comma-separated lists for controlled ID’s, ICAO codes and point names respectively to filter the affected lists.

\bigskip


Left-clicking the Display option cycles through “ALL” (no filtering), “ON-CONTACT” (only tracks \stateref{On Contact} with anyone), “ON-CONTACT-PPOS” (only tracks \stateref{On Contact} with you) and “FREE” (only tracks in the \stateref{Free} state).

\bigskip

Left-clicking the Filter box opens a text entry box to enter a comma-separated list of aerodrome ICAO codes to filter the list. When entered, the list will display a flight only if one of the entered codes is its departure or destination, or the code is found in \tagref{tag:OP TEXT2}.

\bigskip

Left-clicking the Display option toggles between “ALL” (no filtering), “SIMUL+TERM” (not started flight plans filtered), “NOTST+SIMUL” (terminated flight plans filtered) and “SIMUL” (not started and terminated flight plans filtered).
...
Safety Nets and TCT Status…     & Opens the \winref{win:snsw}\\
TCT Status                      & Opens the \winref{win:tctsw}\\
...
%FAST Configuration              & Opens the \winref{menu:ad}\\ 
Runway In Use                   & Opens the \winref{menu:ad}\\
Runway line display…            & Opens the \winref{menu:ad}\\
...
For the Areas data file, the displayed area types can be filtered by right-clicking the View button, which then cycles the displayed areas in the following order (and displays the filter in the button label):

\begin{tabular}{l l}
- TSA   & All TSA area types (yellow)                 \\
- MTCD  & MTCD areas (green) and inhibit areas (red)  \\
- STCA  & STCA areas (green) and inhibit areas (red)  \\
- TCT   & TCT areas (green) and inhibit areas (red)   \\
- DD    & CLAM/RAM inhibit areas (red)                \\
\end{tabular}
...
Opens the \winref{menu:rr}. If the rings are selected on, “x” displays the distance between consecutive rings, otherwise “Off”.
...
The default lenght of the prediction line is 2 minutes, and can be changed for all tracks via the \winref{win:tcw}, or for a single track via the \winref{menu:prl}.
...
For all the filters, it is only possible to filter out \stateref{Unconcerned} tracks. Aircraft with transponder codes 7500, 7600 and 7700 and tracks with an STCA, MSAW, APW, TCT or DUPE alert, highlighted tracks, and tracks subject to Blind Spot function alerts are also excluded from filtering. If a filter is active, its name in the Radar Menu will be shown in \colorref{Selected} color.
...
\begin{enumerate}[label=Line \arabic*]
  \setcounter{enumi}{-1}
  \item \tagref{tag:TCT} \tagref{tag:AIW} \tagref{tag:DUPE} \tagref{tag:EMRG}
...
\begin{longtable}[c]{|l|p{8cm}|p{2cm}|}
  ...
  \colortab{Warning}TC+            & Tactical Controller Tool  Warning                                                                                           & \tagref{tag:TCT}           \\ \hline
...
\begin{figure}[H]
  ...
  \resizebox{\columnwidth}{!}{
    \begin{tikzpicture}
      ...
      \drawtag{0.6cm}{2.7,0}{tct};
      \drawtagdesc{4cm}{2.1,-3.3}{ltct}{R: Toggle TCT Leg};
      \draw[red,very thick] (tct.south west) -- (ltct.north west);

      \drawtag{0.8cm}{4.4,0}{aiw};
      \drawtagdesc{3cm}{4.2,-2}{lext}{R: Acknowledge AIW alert};
      \draw[red,very thick] (aiw) -- (lext);

      \drawtag{0.4cm}{6.7,0}{o};
      \drawtagdesc{4cm}{3,2}{lext}{R: Acknowledge OCM};
      \draw[red,very thick] (o) -- (lext);

      \drawtag{1.3cm}{10.7,0}{alrt};
      \drawtagdesc{3cm}{8,-2}{lext}{R: Open Extended label};
      \draw[red,very thick] (alrt) -- (lext);

      \drawtag{0.4cm}{11.6,0}{f};
...
\capdimage{img/xfr.png}{Transfer Menu}%TODO:check image accuracy
...
\begin{itemize}
  \item “CA” sets “Cleared for approach”
  \item “VA” sets “Cleared for visual approach”
  \item “OS” sets “Own separation” (treated as visual approach by safety nets)
  %\item “OS” sets “Own separation” (FAST indicators hidden, treated as visual approach by safety nets)
\end{itemize}

The “APP Clr” folder contains other available approach types for the aircraft’s arrival runway. If none are defined in the plugin settings, the folder is not displayed in the menu.

\warn{The clearances in the “APP Clr” folder will be displayed as “cleared for approach” to controllers not using this plugin (“OS” will be “cleared for visual approach”)}
...
% \subsection{FAST indicators}\label{menu:fasti}
% 
% 
% Separation and/or gap indicators will be automatically displayed for all defined approaches to all airports set as active for arrivals in EuroScope. The indicators or the sequence orders are not synchronized between controllers.
% 
% The system displays a separation indicator, showing the minimum separation from the preceding aircraft (the greater of minimum surveillance separation and wake turbulence separation), and optionally a gap indicator which shows a manually defined distance gap behind the previous aircraft.
% 
% The gaps are set from the FAST List, and a pre-defined gap is also automatically entered in front of an aircraft with a CAT2 or CAT3 approach clearance.
% 
% \begin{longtable}{|p{3cm}|p{2cm}|p{6cm}|}
%   \hline
%     &  \textbf{Symbol} &  \textbf{Color} \\ \hline
%   \endhead
%   Separation indicator  & (  &  Symbol overtaken: \colorref{FTD Urgency} 
%   
%                                 Selected track: \colorref{Track Highlight} 
%   
%                                 Otherwise: \colorref{FTD Nominal}  \nextrow \\
%   Gap indicator         & [  &  Symbol overtaken: \colorref{GAP Warning} 
%   
%                                 Selected track: \colorref{Track Highlight} 
%   
%                                 Otherwise: \colorref{GAP Nominal}  \nextrow \\
%   \hline
% \end{longtable}
% 
% Right-clicking on a gap indicator opens a confirmation window to remove the gap.
% 
% \bigskip
% 
% The separation indicator is hidden if:
% 
% \begin{itemize}
%   \item a gap is present;
%   \item the aircraft hasn’t overtaken the gap indicator, and;
%   \item the separation indicator is further away from the gap indicator than a pre-defined distance, or;
%   \item the aircraft has an “OS” approach clearance.
% \end{itemize}
% 
% The gap indicator is hidden if:
% 
% \begin{itemize}
%   \item the gap is smaller than the required minimum separation, or;
%   \item the aircraft has an “OS” approach clearance.
% \end{itemize}
% 
% If the aircraft overtakes its separation indicator by a pre-defined distance, the symbol will be displayed with \colorref{FTD Urgency} color and the overtake distance will be displayed next to the symbol.
% 
...
% \subsection{GAP Menu}
% \label{menu:gap}
% \capdimage{img/gapm.png}{GAP Menu}
% 
% The GAP menu is used to set an approach gap or cancel an existing one. If opened from a FAST List row, the gap is for that aircraft only (manual gap), and if opened from the FAST Configuration Window, it applies to all new aircraft entered into that sequence (systematic gap).
% 
% \bigskip
% 
% To set a new gap or change an existing one::
% 
% \begin{enumerate}
%   \item Select a value from the list, or enter a custom value to the edit field at the bottom (values from 0.1 to 40.0 nm are accepted)
% \end{enumerate}
% 
% To cancel an existing gap:
% 
% \begin{enumerate}
%   \item Left-click on “CNL GAP”
% \end{enumerate}
% 
\subsection{NFREQ menu}
\label{menu:nfreq}
\capdimage{img/nfreq.png}{NFREQ Menu}

The NFREQ menu is used to set the “next frequency” value to be sent in datalink departure clearances. Left-click on one of the pre-defined values in the list to set it, or on the empty area at the bottom to type in a value manually.
...
\begin{longtable}{|p{0.25\textwidth}|p{0.25\textwidth}|p{0.25\textwidth}|} \hline
  ...
  \tagref{tag:TOBT}*            & Edit E/TOBT                         & E/TOBT Options              \\ \hline
...
“Font” opens the Font menu to select the list font from three options.
...
\begin{longtable}{|p{0.25\textwidth}|p{0.25\textwidth}|p{0.25\textwidth}|p{0.10\textwidth}|} \hline
  \textbf{Item}             & \textbf{Type} & \textbf{Left Mouse}   & \textbf{Right Mouse} \endhead \hline
    ...
    FX                      & optional  &                           & Acknowledge AMAN highlight  \\ \hline
    STA                     & optional  &                           & Acknowledge AMAN highlight  \\ \hline
    TTLTTG                  & optional  &                           & Acknowledge AMAN highlight  \\ \hline
...
%\subsection{FAST List}\label{list:fast}
%
%The FAST List contains aircraft that have been entered into a defined approach sequence. The list is automatically %displayed whenever there is at least one aircraft in the list. Left-clicking the button at the top right corner %collapses the list to display only the title bar.
%
%\capdimage{img/fastlist.png}{FAST List}
%
%“View” opens the View menu:
%
%\begin{longtable}{p{5cm} p{7.5cm}}
%  Header  & Toggle visibility of the list header line \\
%  Default & Sets field visibility to default          \\
%  <field> & Toggles field visibility                  \\
%\end{longtable}
%
%Left-clicking adjusts only the current list, right-clicking sets the same value to all FAST lists.
%
%\bigskip
%
%“Font” opens the Font menu to select the list font from three options.
...
%\bigskip
%
%The list is sorted according to the FSQ field. Aircraft on missed approach are sorted as if they had a sequence number %of -1 and aircraft already landed as if they had a sequence number of 0.
%
%\bigskip
%
%On the right side of the menu bar, the current minimum surveillance separation and the systematic gap, if any, are %displayed for the runway(s). Left-clicking in that area opens the \winref{win:fastcw}.
%
%\bigskip
%
%The FAST list contains the following fields:
%
%\begin{longtable}{|p{0.10\textwidth}|p{0.25\textwidth}|p{0.25\textwidth}|p{0.25\textwidth}|} \hline
%  \textbf{Item}   & \textbf{Type} & \textbf{Left Mouse}     & \textbf{Right Mouse} \endhead \hline
%    FSQ           & mandatory     & Start re-sequence       & Remove from sequence      \\ \hline
%    C/S           & mandatory     & Open \winref{menu:cs}   &                           \\ \hline
%    ATYP          & optional      &                         & Toggle ATYP highlight     \\ \hline
%    W             & optional      &                         & Toggle WTC highlight      \\ \hline
%    RWY           & optional      & Open runway popup list  &                           \\ \hline
%    GAP           & mandatory     & Open \winref{menu:gap}  & Remove gap                \\ \hline
%    ACF           & optional      & Toggle ACF highlight    & Cancel approach clearance \\ \hline
%    \caption{FAST List Construction}
%  \end{longtable}
%
%To resequence an aircraft behind another, first left-click on the FSQ field of the aircraft to be resequenced. The %mouse cursor changes to a cross. Then left-click on the radar position symbol, track label or the FSQ field of the %aircraft behind which the first aircraft is to be moved.
%
%To cancel the re-sequence operation, press the ESC key or right-click
%
...
\begin{itemize}
    ...
    \item If the tracks are not predicted to be vertically separated within the prediction time, “v=” is displayed near the track whose line contains the separation distance label.
\end{itemize}
...
\begin{longtable}{p{2.5cm} p{10cm}}
    ...
    …Label Fields   & Opens/closes the corresponding Label Fields section\\
\end{longtable}
...
All other flights will be filtered out to display just their track position symbols and history dots unless they have an active STCA, APW, MSAW, AIW or TCT alert or are squawking 7500, 7600 or 7700. Tracks without a valid AFL will be filtered out.
...
All other flights will be filtered out to display just their track position symbols and history dots unless they have an active STCA, APW, MSAW, AIW or TCT alert or are squawking 7500, 7600 or 7700.
...
\note{The DFREQ, NFREQ, NPT, ADT and START options have no effect, as they are not used in the CLD message.}
...
\begin{longtable}{p{2.5cm} p{10cm}}
  Activate    & Sets start time to current time, clears end time and sets the area to active state                    \\
  Deactivate  & Sets the end time to current time, sets the area to inactive state                                    \\
...
\begin{longtable}{p{1.5cm} p{11cm}}
    ...
    FSSR              & Opens the \winref{win:gssrcw} (for formation flights only) \\
    MAC               & Not implemented\\
...
“Font” menu label & Opens the Font menu to select the list font size  \\
Sorting option    & Opens a pop-up menu to select the sorting option: 
...
The Weather Messages Window displays weather related messages. To view the messages, left-click on the desired option button (the “LLF” option is not available).
...
Selecting the “SNOWTAM” or “TAF” option will open the \winref{menu:ad} where the desired stations must be selected. Messages are automatically updated at 15 minute intervals - the status bar at the bottom displays the remaining time to the next automatic update when the mouse cursor is over the window area.
...
\begin{itemize}
    \item Left-click on a message in the list
...
\begin{itemize}
    \item Left-click on a single message
...
\begin{longtable}{p{2.5cm} p{10cm}}
    ...
    TCT                 & TCT Displayed if the TCT function is enabled\\
    BS                  & BS Displayed if the Blind Spot function is enabled\\
    ...
    FAST                & Displayed if the FAST function is enabled. \colorref{Warning} color if AMAN as source and data loading has failed \\
    ...
    SMS                 & Displayed if FAST function is enabled \\
    A-FAST              & Displayed if FAST function is enabled \\
\end{longtable}
...
Allows setting the status for the \winref{tool:STCA}, \winref{tool:MSAW}, \winref{tool:APW}, \winref{tool:AIW}, \winref{tool:tct} and \winref{tool:bs} systems. The “On/Off” buttons control the corresponding system’s status.

\bigskip

Below them, there are four entry boxes for callsigns to exclude specific callsigns from the specified safety nets. The “*” wildcard can be used to match multiple callsigns. It causes all the callsigns that match up to the “*” to be a match (i.e. “ABC*” will match all callsigns that start with “ABC”, but “*ABC” will match all callsigns as any characters after the “*” will be ignored).

\bigskip

Below the callsign fields, there are four SSR code boxes that can be used to exclude specific SSR codes from the specified safety nets.

\bigskip

If there are STCA inhibition areas defined in the areas data file, they will be listed in the area in the bottom part of the window. The area vertical limits (displayed in 100’s of feet or meters+“m” depending on system units) can be edited by clicking on the values, and the area activation can be toggled by clicking on the area status.
...
\subsection{TCT Status Window}
\label{win:tctsw}

\winref{menu:global} STS -> TCT Status...

\bigskip

\capdimage{img/tctsw.png}{TCT Status Window}

The window contains four entry boxes for callsigns to exclude specific callsigns from TCT. The “*” wildcard can be used to match multiple callsigns. It causes all the callsigns that match up to the “*” to be a match (i.e. “ABC*” will match all callsigns that start with “ABC”, but “*ABC” will match all callsigns as any characters after the “*” will be ignored). 

\bigskip

Below the callsign fields, there are four SSR code boxes that can be used to exclude specific SSR codes.

\bigskip

If there are TCT areas or TCT inhibition areas defined in the areas data file, they will be listed in the area in the bottom part of the window. The area vertical limits (displayed in 100’s of feet) can be edited by clicking on the values, and the area activation can be toggled by clicking on the area status.
...
Below the inhibit areas, there are four SSR code boxes that can be used to exclude specific SSR codes from MTCD processing. The entered values must be 2 octal digits, and the system will match codes starting with the entered digits (i.e. “20” will match codes 2000-2077).
...
Finally, there are four entry boxes to exclude specific callsigns. The “*” wildcard can be used to match multiple callsigns. It causes all the callsigns that match up to the “*” to be a match (i.e. “ABC*” will match all callsigns that start with “ABC”.
...
\subsection{FAST Configuration Window}\label{win:fastcw}

\winref{menu:global} STS -> STS -> FAST Configuration
FAST List -> left-click on the area of the menu bar where the MSS and SYS GAP are displayed

\capdimage{img/fastcw.png}{FAST Configuration Window}

The FAST Configuration Window is used to change approach-specific settings. If an approach has more than one possible minimum surveillance separation defined, left-clicking on the button in the MSS column cycles through them. Left-clicking on the button in the Sys GAP column opens the GAP menu to set up a systematic gap to be applied for all new aircraft entered into that approach sequence. Right-clicking removes any previously defined gap.
...
\subsection{Group SSR Code Window}\label{win:gssrcw}

“Open Group SSR Code Window” label function 
“Open Group SSR Code Window / ASSR menu” label function 
Left-click FSSR button in Flight Plan Window

\capdimage{img/gssrcw.png}{Group SSR Code Window}

The Group SSR Code Window displays the aircraft belonging to the formation, and their assigned transponder codes. Left-click on a code to open the \winref{menu:assr} to assign a new code.
...
The Flight Leg line is one pixel thick, except when it is showing only SAP conflicts (opened from the SAP Window) or showing also MTCD information (any conflicting aircraft legs). In that case the FLEG for the referenced flight is three pixels thick and the other FLEGs one pixel thick.
...
\section{Tactical Controller Tool (TCT) Leg}\label{func:tct}

The TCT leg displays the predicted trajectories of the selected aircraft and the conflicting aircraft up to the closest point of approach. If the selected aircraft is involved in more than one conflict, the displayed one for the selected aircraft is the conflict where the time to start of conflict is the least.

\bigskip

The leg is \colorref{TCT Leg} color until the start of the conflict, and then \colorref{TCT Leg Embellished} until the closest point of approach. On the conflicting aircraft’s leg, a label is displayed in \colorref{TCT Label} color. It displays the minimum distance and time to that point (closest point of approach).

\bigskip

When TCT legs are being displayed, all minimum separation tools (except for tracks with STCA alert), velocity vectors (except for tracks with STCA alert) and flight legs are temporarily hidden.
...
\begin{longtable}{|p{4.5cm}|p{1.5cm}|p{4.5cm}|}
  ...
  \nextrow \label{AMAN Data Highlight} AMAN Data Highlight              & \cellcolor{AMAN Data Highlight}     & Labels                                      \\ \hline
  ...
  \nextrow \label{FSQ SI} FSQ SI                                        & \cellcolor{FSQ SI}                  & Labels                                      \\ \hline
  ...
  \nextrow \label{TCT Warning} TCT Warning                              & \cellcolor{TCT Warning}             & Labels                                      \\ \hline
  ...
  \nextrow \label{FTD Nominal} FTD Nominal                              & \cellcolor{FTD Nominal}             & FAST separation indicators                  \\ \hline
  \nextrow \label{FTD Urgency} FTD Urgency                              & \cellcolor{FTD Urgency}             & FAST separation indicators and labels when overtaken \\ \hline
  \nextrow \label{GAP Nominal} GAP Nominal                              & \cellcolor{GAP Nominal}             & FAST gap indicators                         \\ \hline
  \nextrow \label{GAP Warning} GAP Warning                              & \cellcolor{GAP Warning}             & FAST gap indicators when overtaken          \\ \hline
  ...
  \nextrow \label{SEP Tool} SEP Tool                                    & \cellcolor{SEP Tool}                & Minimum separation tools 8 and greater      \\ \hline
  ...
  \nextrow \label{TCT Label} TCT Label                                  & \cellcolor{TCT Label}               & TCT leg labels                              \\ \hline
  \nextrow \label{TCT Leg} TCT Leg                                      & \cellcolor{TCT Leg}                 & TCT leg part before conflict start          \\ \hline
  \nextrow \label{TCT Leg Embellished} TCT Leg Embellished              & \cellcolor{TCT Leg Embellished}     & TCT leg from conflict start to CPA          \\ \hline
  ...
  \nextrow \label{BS Aged} BS Aged                                      & \cellcolor{BS Aged}                 & CFL menu Blind Spot function results        \\ \hline
  \nextrow \label{BS Crossed Level} BS Crossed Level                    & \cellcolor{BS Crossed Level}        & CFL menu Blind Spot function results        \\ \hline
  \nextrow \label{BS No Conflict} BS No Conflict                        & \cellcolor{BS No Conflict}          & CFL menu Blind Spot function result         \\ \hline
  \nextrow \label{BS Unknown} BS Unknown                                & \cellcolor{BS Unknown}              & CFL menu Blind Spot function result         \\ \hline
  \nextrow \label{BS Warning} BS Warning                                & \cellcolor{Warning}                 & CFL menu Blind Spot function result         \\ \hline
